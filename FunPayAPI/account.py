...
class Account:
    ...
    def get_lot_page(self, lot_id: int, locale: Literal["ru", "en", "uk"] | None = None):
        ...
        for param_item in parser.find_all("div", class_="param-item"):
            if param_name := param_item.find("h5"):
                ...
                elif param_name in ("Подробное описание", "Докладний опис", "Detailed description"):
    ...
    def get_order(self, order_id: str, locale: Literal["ru", "en", "uk"] | None = None) -> types.Order:
        ...
        if (span := parser.find("span", {"class": "text-warning"})) and span.text in ("Возврат", "Повернення", "Refund"):
        ...
        lot_id = None
        if (lot_link := parser.find("a", href=re.compile(r"lots/offer\\?id=\\d+"))):
            try:
                lot_id = int(lot_link["href"].split("id=")[-1])
    ...
    def calc(self, subcategory_type: enums.SubCategoryTypes, subcategory_id: int, price: int | float = 1000):
        ...
        try:
            min_price, min_price_currency = json_resp.get("minPrice").rsplilt(" ", maxsplit=1)
        ...
        for i in games_divs:
            ...
            for j in subcategories_divs:
                ...
                for k in subcategories:
                    ...
                    stype = types.SubCategoryTypes.CURRENCY if "chips" in link else \
                        types.SubCategoryTypes.COMMON
